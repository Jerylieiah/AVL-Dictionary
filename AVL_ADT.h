#ifndef AVLDICTIONARY_AVL_ADT_H
#define AVLDICTIONARY_AVL_ADT_H
...
private:
    ...
    NODE<TYPE>  *dltRightBalance (NODE<TYPE>  *root,
                                  bool&        shorter);
    void         _retrieve        (KTYPE        key,
                                  NODE<TYPE>  *root,
                                  vector<wordNode> &wordList);
    void        _retrieveAll (KTYPE key, NODE<TYPE> *root, vector<wordNode> &wordList);
...
public:
    ...
    vector<wordNode>  AVL_Retrieve (KTYPE  key);
    void  AVL_Traverse (/*void (*process)(TYPE  dataProc)*/); //in-order

    bool  AVL_Empty    (void);
    bool  AVL_Full     (void);
    int   AVL_Count    (void);
    int   AVL_Return_Count ();
    int   AVL_Update_Freq  ();
...
//	Statements
        ...
        //  new data >= root data
    {
        ...
        root->data.numTimes++;
        root->data.frequency = root->data.numTimes / count;
    }
    return root;
}	//  _insert
...
template <class TYPE, class KTYPE>
vector<wordNode>   AvlTree<TYPE, KTYPE>
...
//	Statements
    if (root)
    {
        ...
        {
            if ( key[i] == root->data.key[i] )
                check++;
        }

        if (check == key.size())
        {
            wordList.push_back(root->data);
            _retrieveAll(key, root->left, wordList);
            return;
        }
        ...
        else
        {
            if ( key < root->data.key )
                _retrieve(key, root->left, wordList);
            else if ( key > root->data.key )
                _retrieve(key, root->right, wordList);
        }
    } // if root
}	//  _retrieve
...
template<class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>
::  _retrieveAll (KTYPE key, NODE<TYPE> *root, vector<wordNode> &wordList)
{
    if ( root != nullptr )
    {
        _retrieveAll(key, root->left, wordList);
        ...
        _retrieveAll(key, root->right, wordList);
    }
}
...
//	Statements
    if (root)
    {
        ...
        root->data.frequency = root->data.numTimes / count * 100;
        cout << root->data.key << " (" << root->data.numTimes << ") (" << root->data.frequency << "%)" << endl;
        //process     (root->data);
...
template<class TYPE, class KTYPE>
int AvlTree<TYPE, KTYPE> ::AVL_Return_Count()
{
    return count;
}

template<class TYPE, class KTYPE>
int AvlTree<TYPE, KTYPE>::AVL_Update_Freq()
{

    return 0;
}

#endif //AVLDICTIONARY_AVL_ADT_H